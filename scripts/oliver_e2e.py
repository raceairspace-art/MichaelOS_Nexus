...
def request_json(url:str,*,method:str="GET",payload:dict|None=None,timeout:int=90)->dict:
    data=None;headers={"Accept":"application/json","User-Agent":"MichaelOS-Oliver-E2E/3.0"};bypass=os.environ.get("VERCEL_AUTOMATION_BYPASS_SECRET","")
    if bypass:
        headers["x-vercel-protection-bypass"]=bypass
        headers["x-michaelos-automation-bypass"]=bypass
    ...
    try:
        with urllib.request.urlopen(request,timeout=timeout) as response:
            ...
            if response.status<200 or response.status>=300:raise RuntimeError(f"HTTP {response.status}: {body[:1000]}")
            return json.loads(body)
    except urllib.error.HTTPError as exc:
        body=exc.read().decode("utf-8",errors="replace");raise RuntimeError(f"HTTP {exc.code} {url}: {body[:2000]}") from exc
    except urllib.error.URLError as exc:raise RuntimeError(f"Network error calling {url}: {exc}") from exc
...
def market_url(base_url:str,symbol:str,session_date:str|None=None)->str:
    ...
    if session_date:query["date"]=session_date
...
def validate_market(symbol:str,market:dict,expected_date:str|None=None)->None:
    if market.get("symbol")!=symbol:raise RuntimeError(f"{symbol}: market response symbol mismatch: {market.get('symbol')!r}")
    if market.get("phase")!="decision":raise RuntimeError(f"{symbol}: expected decision phase, got {market.get('phase')!r}")
    bars=market.get("bars")
    if not isinstance(bars,list) or not bars:raise RuntimeError(f"{symbol}: decision response has no bars")
    if market.get("outcome") is not None:raise RuntimeError(f"{symbol}: decision response leaked outcome data")
    if expected_date and market.get("sessionDate")!=expected_date:raise RuntimeError(f"{symbol}: requested {expected_date}, server returned {market.get('sessionDate')}")
    if not isinstance(market.get("engineReview"),dict):raise RuntimeError(f"{symbol}: deterministic Oliver Decision Engine review missing")
...
def validate_review(symbol:str,response:dict)->dict:
    ...
    if not isinstance(review,dict):raise RuntimeError(f"{symbol}: AI review response missing review object: {response}")
    for field in ("overallGrade","wouldTrade","direction","overallQuality","confidence","rationale"):
        if field not in review:raise RuntimeError(f"{symbol}: AI review missing {field}: {review}")
    return review

def main()->int:
    if len(sys.argv)!=2:print("usage: oliver_e2e.py <deployment-base-url>",file=sys.stderr);return 2
    ...
    if auth.get("bypass") is not True:raise RuntimeError(f"Nexus auth layer did not recognize automation bypass: {auth}")
    print("      PASS authenticated automation bypass")
    print("[2/7] Resolve a trading session and verify deterministic engine")
    aapl=request_json(market_url(base_url,"AAPL"));validate_market("AAPL",aapl);session_date=aapl.get("sessionDate")
    ...
    for symbol in SYMBOLS[1:]:
        market=request_json(market_url(base_url,symbol,session_date));validate_market(symbol,market,session_date);markets[symbol]=market;print(f"      PASS {symbol}: bars={len(market['bars'])}, engine={market['engineReview'].get('overallGrade')}")
...
if __name__=="__main__":
    ...
    except Exception as exc:print(f"E2E FAIL: {exc}",file=sys.stderr);raise
